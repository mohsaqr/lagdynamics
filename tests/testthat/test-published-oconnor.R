...
# B. Every single output matrix lagseq's classical engine produces is
...
# that lagseq replaced (clean-room). If lagseq's independent
...
test_that("O'Connor 1999: lagseq reproduces published TRANSITION COUNTS exactly", {
...
test_that("O'Connor 1999: lagseq reproduces published EXPECTED FREQUENCIES (4dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
  expect_equal(unname(fit$exp), unname(oconnor_couple$expected),
               tolerance = 1e-3)
})

test_that("O'Connor 1999: lagseq reproduces published TRANSITIONAL PROBABILITIES (4dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
...
test_that("O'Connor 1999: lagseq reproduces LR chi-square test", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
...
test_that("O'Connor 1999: lagseq reproduces published ADJUSTED RESIDUALS (3dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
  expect_equal(unname(fit$adj_res), unname(oconnor_couple$adj_res),
               tolerance = 1e-3)
})

test_that("O'Connor 1999: lagseq reproduces published P-VALUES (4dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
  expect_equal(unname(fit$p), unname(oconnor_couple$adj_p),
               tolerance = 1e-3)
})

test_that("O'Connor 1999: lagseq reproduces published YULE'S Q (3dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
...
test_that("O'Connor 1999: lagseq reproduces published KAPPAS (4dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
  expect_equal(unname(fit$kappa), unname(oconnor_couple$kappa),
               tolerance = 1e-3)
})

test_that("O'Connor 1999: lagseq reproduces published KAPPA Z-SCORES (3dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
  expect_equal(unname(fit$kappa_z), unname(oconnor_couple$kappa_z),
               tolerance = 1e-3)
})

test_that("O'Connor 1999: lagseq reproduces published KAPPA P-VALUES (4dp)", {
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
...
test_that("O'Connor 1999: chi-square primitive equivalence at 1e-12", {
  ...
  # adjusted residuals as lagseq does on the original sequence.
  fit <- lsa(oconnor_couple$sequence, engine = "classical")
  suppressWarnings({
