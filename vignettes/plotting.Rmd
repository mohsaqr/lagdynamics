...
`lagdynamics` has one plotting verb, `plot(fit, type = )`, plus dedicated
...
follow the wider Transition Network Analysis (TNA) convention -- **blue =
more than chance**, **red = less** (avoided edges dashed) -- so an `lsa`
...
transition network, drawn in the Transition Network Analysis (TNA) style by
`cograph::splot()`: coloured nodes, a per-node initial-probability ring, and
weighted directed edges. This shows what happens, where the residual network
...
```{r barrel, eval = has("ggplot2"), fig.height = 7}
...
```{r bayes-barrel, eval = has("ggplot2"), fig.height = 7}
